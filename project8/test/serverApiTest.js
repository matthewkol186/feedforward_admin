...
describe('CS142 Photo App API - ', function () {
        ...
        it('has an entry for each of the users', function (done) {
            async.each(cs142Users, function (realUser, callback) {
                ...
                assert.strictEqual(_.countBy(userList, '_id')[user._id], 1, 'Multiple users with id:' + user._id);
								// the model won't have the numCounts property on the user object, so add one to extraProps so this doesn't make a mistake during testing
                var extraProps = _.difference(Object.keys(removeMongoProperties(user)), userListProperties);
                assert.strictEqual(extraProps.length, 2, 'user object has extra properties: ' + extraProps);
                callback();
            }, done);
        });
    ...
    describe('test /photosOfUser/:id', function (done) {
                        ...
                        _.forEach(real_photos, function (real_photo) {
                            ...
                            if (real_photo.comments) {
                                ...
                                _.forEach(real_photo.comments, function (real_comment) {
                                    ...
                                    assert.strictEqual(extraProps2.length, 0);
		...
		describe('test /commentsBy/:id', function (done) {
        var userList;
        var cs142Users = cs142models.userListModel();
        ...
        it('can get all photos containing comments by a particular user from /commentsBy/:id', function (done) {
						var allPhotos = [];
						for(var userIndex in cs142Users) {
							allPhotos = allPhotos.concat(cs142models.photoOfUserModel(cs142Users[userIndex]._id));
						}
            async.each(cs142Users, function (realUser, callback) {
                // validate the the user is in the list once
                var user = _.find(userList, {
                ...
                http.get({
                    ...
                    response.on('end', function () {
                        ...
                        photos = JSON.parse(responseBody);
												...
												// JSON parse trick to ensure that we don't modify the original allPhotos variable :)
                        let real_photos = JSON.parse(JSON.stringify(allPhotos.filter(function(photo) {
													if(!photo.comments) {
														return false;
													}
													for(var commentIndex in photo.comments) {
														if(photo.comments[commentIndex].user._id === realUser._id) {
															return true;
														}
													}
													return false;
												})));
												// filter comments by the ones that were written by the user
												for (var photo in real_photos) {
													real_photos[photo].comments = real_photos[photo].comments.filter(function(comment) {
														return comment.user._id === realUser._id;
													});
												}

                        assert.strictEqual(real_photos.length, photos.length, 'wrong number of photos returned for user ID '+user._id);
                        _.forEach(real_photos, function (real_photo) {
                            var matches = _.filter(photos, {file_name: real_photo.file_name});
                            assert.strictEqual(matches.length, 1, ' looking for photo ' +
                                real_photo.file_name);
                            ...
                            if (real_photo.comments) {
                                ...
                                _.forEach(real_photo.comments, function (real_comment) {
                                    var comment = _.find(photo.comments, {comment: real_comment.comment});
                                    assert(comment);
																		// commented this out because the comment only needs to have the user_id property, not the full user property
                                    /*var extraProps2 = _.difference(Object.keys(removeMongoProperties(comment)), commentProperties);
        ...
        it('can return a 400 status on an invalid id to commentsBy', function (done) {
            http.get({
                hostname: host,
                port: port,
                path: '/commentsBy/1',
